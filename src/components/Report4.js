...
class Report4 extends Component {
    ...
    axios.get(testUrl + "/chartData").then(res => {
      ...
      res.data.forEach(function(item, index) {
        ...
        if (item.Subject === "#agile") {
          if (item.Level === "Beginner") {agBeg = agBeg + 1;
          } else if (item.Level === "Intermediate") {agImt = agImt + 1;
          }

          if (item.Attendees === 0) {vag0 = vag0 + 1;
          } else if (item.Attendees > 10) {vag10more = vag10more + 1;
          } else{vag10 = vag10 + 1;
          }
         ...
         // console.log("0: " + vag0 + "  -10: " + vag10 + " 10+: " + vag10more)
          ...
          if (item.Level === "Beginner") {cBeg = cBeg + 1;
          } else if (item.Level === "Intermediate") {cImt = cImt + 1;
          }

          if (item.Attendees === 0) {vc0 = vc0 + 1;
          } else if (item.Attendees > 10) {vc10more = vc10more + 1;
          } else {vc10 = vc10 + 1;
          }
          ...
          if (item.Level === "Beginner") {aiBeg = aiBeg + 1;
          } else if (item.Level === "Intermediate") {aiimt = aiimt + 1;
          }
          if (item.Attendees === 0) {vai0 = vai0 + 1;
          } else if (item.Attendees > 10) {vai10more = vai10more + 1;
          } else {vai10 = vai10 + 1;
          }
      ...
      this.setState({ totalBeg: agBeg + cBeg + aiBeg, totalImt: agImt + cImt + aiimt });
      this.setState({ TotalAtte: totalA, TotalEvents: agBeg + cBeg + aiBeg + agImt + cImt + aiimt});
      this.setState({ TotalAgileAtt: agTotalAtt, TotalAgileEvents: agBeg + agImt});
      this.setState({ TotalAIAtt: aiTotalAtt, TotalAIEvents: aiBeg + aiimt});
      this.setState({ TotalcloudAtt: cTotalAtt, TotalcloudEvents: cBeg + cImt});
      this.setState({ tol0: vai0 + vag0 + vc0, tol10: vai10 + vag10 + vc10, tol10more: vai10more + vag10more + vc10more});
      ...
      if (agBeg + cBeg + aiBeg > agImt + cImt + aiimt) {this.setState({ yValue: agBeg + cBeg + aiBeg + 3 });
      } else {this.setState({ yValue: agImt + cImt + aiimt + 3 });
    }
    });
  }
  ...
  handleChange = e => {
    ...
    if (e === "Cloud") {
      this.setState({ tagBeg: this.state.cloudBeg, tagImt: this.state.cloudImt });
      this.setState({ tagTotalAtt: this.state.TotalcloudAtt, tagTotalEvent: this.state.TotalcloudEvents });
      ...
      this.setState({ tagTotalAtt: this.state.TotalAIAtt, tagTotalEvent: this.state.TotalAIEvents});
      ...
      this.setState({ tagBeg: this.state.agileBeg, tagImt: this.state.agileImt });
      this.setState({ tagTotalAtt: this.state.TotalAgileAtt, tagTotalEvent: this.state.TotalAgileEvents});
      ...
      <Row>
        <Col sm={{ span: gobalSize, offset: gobalOffset }}>
          ...
          <Row>
            ...
            <Chart
                dataValues={[this.state.tol0, this.state.tol10, this.state.tol10more, 0]}
            ...
            <Col sm={5} hidden={this.state.hidden}>
              <Chart
                ...
                y={this.state.yValue}
              />
              <p>Total events held for {this.state.tag}: {this.state.tagTotalEvent}</p>
              <Chart
                dataValues={[this.state.tag0, this.state.tag10, this.state.tag10more, 0]}
                chartTitle={this.state.tag + " event attendees range"}
                y={this.state.yValue}
              />
              <p>Total attendees for {this.state.tag} events: {this.state.tagTotalAtt}</p>
            </Col>
          </Row>
        </Col>
      </Row>
